...
| Reader\Xlsx | `load` (paths and stream-wrapper URLs), `setReadDataOnly`, `canRead` | implements `IReader`; wrapper sources are staged through a local temp file before the native open |
